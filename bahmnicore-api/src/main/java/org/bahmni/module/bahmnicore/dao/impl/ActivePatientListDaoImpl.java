...
public class ActivePatientListDaoImpl implements ActivePatientListDao {
    ...
    private static String DISCHARGE_PATIENT_CONCEPT_NAME = "Discharge Patient";
    ...
    public ResultList getPatientsForAdmission() {
        ...
        "and v.patient_id not in (select patient_id from bed_patient_assignment_map where date_stopped is null) ";
    ...
    @Override
    public ResultList getAdmittedPatients() {
        String query = "select distinct pn.given_name, pn.family_name, pi.identifier, concat(\"\",p.uuid), concat(\"\",v.uuid) " +
                "from bed_patient_assignment_map bp " +
                "join visit v on bp.patient_id=v.patient_id and v.date_stopped is null " +
                "join person_name pn on bp.patient_id = pn.person_id and pn.voided = 0 " +
                "join patient_identifier pi on bp.patient_id = pi.patient_id " +
                "join person p on p.person_id = bp.patient_id " +
                "where bp.date_stopped is null ";
    ...
    @Override
    public ResultList getPatientsForDischarge() {
        String query = "select distinct pn.given_name, pn.family_name, pi.identifier, concat(\"\",p.uuid), concat(\"\",v.uuid) " +
                "from visit v " +
                "join person_name pn on v.patient_id = pn.person_id and pn.voided = 0 " +
                "join patient_identifier pi on v.patient_id = pi.patient_id " +
        ...
        SQLQuery sqlQuery = sessionFactory.getCurrentSession().createSQLQuery(query);
        sqlQuery.setParameter("conceptName", DISCHARGE_PATIENT_CONCEPT_NAME);
        return new ResultList(sqlQuery.list());

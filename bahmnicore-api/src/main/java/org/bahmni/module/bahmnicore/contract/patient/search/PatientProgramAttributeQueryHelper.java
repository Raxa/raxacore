...
public class PatientProgramAttributeQueryHelper {
	...
	public String appendToJoinClause(String join){
		return join + " left outer join patient_program pp on pat.patient_id = pp.patient_id and pp.voided=0"
				+ " inner join patient_program_attribute ppa on pp.patient_program_id = ppa.patient_program_id and ppa.voided=0"
				+ " inner join program_attribute_type ppt on ppa.attribute_type_id = ppt.program_attribute_type_id and ppa.attribute_type_id ="+programAttributeTypeId.intValue();
	}

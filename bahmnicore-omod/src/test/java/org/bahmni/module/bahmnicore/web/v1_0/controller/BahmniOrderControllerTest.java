...
public class BahmniOrderControllerTest {
    ...
    public void returnLatestObservationsForAllOrdersInOrderType() throws Exception {
        ...
        when(bahmniOrderService.getLatestObservationsAndOrdersForOrderType("patientUuid", Arrays.asList(concept), null, null, "OrderTypeUuid")).thenReturn(Arrays.asList(bahmniOrder));

        BahmniOrderController bahmniOrderController = new BahmniOrderController(conceptService, bahmniOrderService);
        List<BahmniOrder> bahmniOrders = bahmniOrderController.get("patientUuid", Arrays.asList("Weight"), null,null, "OrderTypeUuid");
    ...
    public void returnLatestObservationsForOrder() throws Exception {
        ...
        verify(bahmniOrderService, never()).getLatestObservationsAndOrdersForOrderType("patientUuid", Arrays.asList(concept), null, null, "someUuid");
        assertEquals(1, bahmniOrders.size());
    }
}